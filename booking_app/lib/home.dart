...
class _HomeState extends State<Home> {
        ...
        return Dismissible(
            ...
            onDismissed: (direction) async {
              if (direction == DismissDirection.startToEnd) {
                setState(() {});
                _images = await Imagemservice.pegarImagensPeloIdPropriedade(
                    propriedades[index].id!);
                ...
                Timer(Duration(seconds: 5), () {
                  if (!isUndone) {
                    Reservaservice.deletarReservasAssociadaPropriedade(
                        lastRemoved.id!);
                    Propriedadeservice.deletarPropriedade(lastRemoved.id!)
                        .then((_) {
                      Propriedadeservice.buscarPropriedadesDeUsuario(
                              widget.usuario.id!)
                          .then((p) {
                        setState(() {
                          propriedades = p;
  ...
  _salvarPropriedade() async {
    ...
    Endereco enderecoBancoDeDados =
        await Enderecoservice.precisaSalvarEnderecoNoBanco(enderecoViaApi);
    //Cria a propriedade e salva no banco de dados
    Propriedade propriedade = Propriedade(
        address_id: enderecoBancoDeDados.id!,
        user_id: widget.usuario.id!,
        title: titleController.text,
        description: descriptionController.text,
        price: double.parse(priceController.text),
        max_guest: int.parse(maxguestController.text),
        number: int.parse(numberController.text),
        complement: complementController.text,
        thumbnail: _images![0].path);
    propriedade.id = await Propriedadeservice.criarPropriedade(
        Propriedade.fromPropriedadeToJson(propriedade));
  ...
  void initState() {
    super.initState();
    Propriedadeservice.buscarPropriedadesDeUsuario(widget.usuario.id!)
        .then((p) {
      propriedades = p;
      setState(() {});
    });
